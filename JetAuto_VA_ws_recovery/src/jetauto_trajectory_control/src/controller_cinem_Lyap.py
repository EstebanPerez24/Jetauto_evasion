...
class PoseControl:
    ...
    def plot(self):
        ...
        plt.figure(figsize=(win_size_y, win_size_y))
        plt.plot(self.time, (self.goalx - self.x_sim),label='Error en X')
        plt.plot(self.time, (self.goaly - self.y_sim),label='Error en Y')
        #plt.plot(self.time,self.theta_sim,label='Esimado')
        plt.xlabel('x [m]')
        plt.ylabel('t [s]')
        plt.title('Errores de posici[on')
        plt.grid(True)
        plt.legend()
        #circle = plt.Circle((self.x_sim[-1], self.y_sim[-1]), radius=0.05, color='tab:orange', alpha=1)
        #plt.gca().add_patch(circle)
    ...
    def run(self):
        ...
        for i in range(0,len(self.goalx)):
            ...
            acx = (self.kp * math.tanh(e_x) + self.goalx_d[i])
            acy = (self.kp * math.tanh(e_y) + self.goaly_d[i])
            acw = (self.kp * 1.5 * e_theta + self.goaltheta_d[i])
