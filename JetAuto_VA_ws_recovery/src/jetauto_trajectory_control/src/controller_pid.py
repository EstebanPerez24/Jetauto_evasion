...
class PoseControl:
    ...
    def run(self):
        ...
        for i in range(0,len(self.goalx)):
            ...
            self.etheta[0] = self.goaltheta[i] - self.theta
            self.etheta[0] = (self.etheta[0] + np.pi) % (2*np.pi) - np.pi
                       
            self.acx = self.acx + self.A0x * self.ex[0] + self.A1x * self.ex[1] + self.A2x * self.ex[2]
            self.acy = self.acy + self.A0y * self.ey[0] + self.A1y * self.ey[1] + self.A2y * self.ey[2]
            self.acw = self.acw + self.A0theta * self.etheta[0] + self.A1theta * self.etheta[1] + self.A2theta * self.etheta[2]
