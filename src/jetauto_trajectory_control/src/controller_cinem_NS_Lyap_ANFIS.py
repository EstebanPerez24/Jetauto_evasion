...
class PoseControl:
    def __init__(self):
        ...
        if self.guardar_datos:
            ...
            directory = os.path.join(package_path,'datos','NS_Lyap_ANFIS')
            if not os.path.exists(directory):
                os.makedirs(directory)  # Create the directory if it doesnt exist
            self.file_name = os.path.join(directory, "NS_Lyap_ANFIS_{}.txt".format(path_type))
    ...
    def get_inv_Jacobian_ANFIS(self,th):
        th1 = th + np.pi/4
        r2 = np.sqrt(2)
        J_inv_ANFIS = np.array([[r2 * np.cos(th1) , r2 * np.sin(th1), -(self.lx + self.ly)],
    ...
    def run(self):
        ...
        while i < len(self.goalx) and not rospy.is_shutdown():
            ...
            if self.usar_fuzzy:
                ...
                J_inv_ANFIS = self.get_inv_Jacobian_ANFIS(self.theta)
                w = np.dot(J_inv_ANFIS,u)/self.r
