...
class PoseControl:
    def __init__(self):
        ...
        if self.guardar_datos:
            ...
            directory = os.path.join(package_path,'datos','PI_ANFIS')
            if not os.path.exists(directory):
                os.makedirs(directory)  # Create the directory if it doesnt exist
            self.file_name = os.path.join(directory, "PI_ANFIS_{}.txt".format(path_type))
    ...
    def run(self):
        ...
        while i < len(self.goalx) and not rospy.is_shutdown():
            ...
            if self.usar_fuzzy:
                ...
                self.acw = -self.engine_w.output_variable("out1").value

                # Rotar al marco del robot usando theta
                self.acx = vx_lidar * np.cos(self.theta) - vy_lidar * np.sin(self.theta)
                self.acy = vx_lidar * np.sin(self.theta) + vy_lidar * np.cos(self.theta)
