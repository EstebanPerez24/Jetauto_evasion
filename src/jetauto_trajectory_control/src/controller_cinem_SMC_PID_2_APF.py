...
class PoseControl:
    def __init__(self):
        ...
        self.zeta = 1.1547
        self.eta = 0.4
        self.dstar = 0.5    #0.5, 0.2
        self.Qstar = 0.24
        self.v_max = 0.125 #0.175
        self.w_max = 6.0
        self.position_accuracy = 0.05 #0.15 rectangle, 0.05 lemniscate
        ...
        if self.guardar_datos:
            ...
            directory = os.path.join(package_path,'datos','SMC_TF_APF')
            if not os.path.exists(directory):
                os.makedirs(directory)  # Create the directory if it doesnt exist
            self.file_name = os.path.join(directory, "SMC_TF_APF_{}.txt".format(path_type))
        ...
        if self.trajectory_name == "rectangle":
            ...
            if abs(self.goalx[self.evasion_start_index + 1] - self.goalx[self.evasion_start_index]) > \
               abs(self.goaly[self.evasion_start_index + 1] - self.goaly[self.evasion_start_index]):
                steps_needed = int(np.ceil((obstacle_size + 0.45) / avg_distance_x)) if avg_distance_x > 0 else 10
            else:
                steps_needed = int(np.ceil((obstacle_size + 0.45) / avg_distance_y)) if avg_distance_y > 0 else 10
        else:
            steps_needed = int(np.ceil((obstacle_size + 0.45) / avg_distance_x)) if avg_distance_x > 0 else 10
    ...
    def run(self):
        ...
        for i in range(0,len(self.goalx)):
            ...
            if np.any(repulsive_force != 0) and self.aux_apf + 10 <= i:  # Se detecta obstáculo
            ...
            if self.using_apf:  # Si APF ya fue activado, permanece en APF hasta alcanzar el objetivo
                ...
                w_ac = np.clip(self.kp_theta * error_theta, -self.w_max, self.w_max)
                v_ref = min(np.linalg.norm(-total_force), self.v_max)
                vx_ac = v_ref * np.cos(self.theta)
                vy_ac = v_ref * np.sin(self.theta)
                
                ac_vector = np.array([[vx_ac],[vy_ac],[w_ac]])
